<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="报告编号" prop="baogaobianhao">
            <el-input v-model="ruleForm.baogaobianhao" 
                placeholder="报告编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.baogaobianhao" label="报告编号" prop="baogaobianhao">
              <el-input v-model="ruleForm.baogaobianhao" 
                placeholder="报告编号" readonly></el-input>
          </el-form-item>
        </div>
...
export default {
  ...
  created() {
	this.ruleForm.zhenduanshijian = this.getCurDate()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jianchabaogao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jianchabaogaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
